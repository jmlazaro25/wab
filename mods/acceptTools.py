...
class module:
    def __init__( self, center, layer, config='full_90' ):
        ...
        shape, angle = config.split( '_' )[ 0 ], float(
            config.split( '_' )[ 1 ]
            )
        ...
        if angle != 0:
            self.corners = [
                rotate( corner, angle ) for corner in self.corners
                ]
        ...
        # Connect the dots
        ###########################
        self.sides = [ [ self.corners[ k ], self.corners[ k + 1 ] ]
                       for k in range( len( self.corners ) - 1 ) ]
        self.sides.append( [ self.corners[ -1 ], self.corners[ 0 ] ]
                          ) # connect back to first corner

    def contains( self, xy ):
        #if xyz[2] != self.cz: return False # don't bother if not at same z removing 3D-ness
        ray = [ ( xy[ 0 ], xy[ 1 ] ),
                ( xy[ 0 ] + 500, xy[ 1 ] + 500 ) ] # draw a ray out from xyz
        crossings = 0
            ...
            for center in self.centers:
                ...
                if config == 'v12' or config == 'config_3':
                    self.layers[ layer ].append(
                        module( center, layer, 'full_90' )
                        )
                if config == 'config_0':
                    self.layers[ layer ].append(
                        module( center, layer, 'full_0' )
                        )
                if config == 'config_1'  or\
                    config == 'config_2' or config == 'xLR' or\
                    config == 'config_6' or config == 'square':
                    if mod_count < 7:
                        ...
                        self.layers[ layer ].append(
                            module( center, layer, 'half_0' )
                            )
                    elif mod_count < 11:
                        self.layers[ layer ].append(
                            module( center, layer, 'half_180' )
                            )
                    elif mod_count < 14:
                        self.layers[ layer ].append(
                            module( center, layer, 'three_180' )
                            )
                    else:
                        self.layers[ layer ].append(
                            module( center, layer, 'three_0' )
                            )
