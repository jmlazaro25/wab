...
lineStyles = { 'kSolid': 1, 'kDashed': 2, 'kDotted': 3, 'kDashDotted': 4}
...
class TreeProcess:
    ...
    def __init__(
        ...
        if self.tree == None:
            ...
            if self.batch:
                self.tmp_dir = '%s/%s' % (
                    scratch_dir, os.environ[ 'LSB_JOBID' ]
                    )
            else:
...
class TreeMaker:
    ...
    def addBranch( self, rtype, default_value, branch_name ):
        ...
        self.branches_info[ branch_name ] = {
            'rtype': rtype,
            'default': default_value
            }
        self.branches[ branch_name ] = np.zeros( 1, dtype=rtype )
        if str( rtype
               ) == "<type 'float'>" or str( rtype ) == "<class 'float'>":
            self.tree.Branch(
                branch_name, self.branches[ branch_name ], branch_name + "/D"
                )
        elif str( rtype ) == "<type 'int'>" or str( rtype ) == "<class 'int'>":
            self.tree.Branch(
                branch_name, self.branches[ branch_name ], branch_name + "/I"
                )
...
def parse( nolist=False ):
    ...
    parser.add_argument(
        ...
        dest='nolist',
        help=
        "return things without lists (to make things neater for 1 sample runs"
        )
    parser.add_argument(
        '-s',
        '--start',
