...
##########################
def nearPhotonInfo(
        trackingHitList, g_trajectory, returnLayer=True, returnNumber=True
    ):
    ...
    for hit in trackingHitList:
        ...
        if physTools.dist(
            physTools.pos( hit )[ :2 ],
            g_trajectory[ physTools.ecal_layer( hit ) ]
            ) < physTools.cellWidth:
            n += 1
...
def findStraightTracks(hitlist, etraj_ends, ptraj_ends,\
    ...
    for hit in hitlist: #Go through all hits, starting at the back of the ecal
        ...
        if len( track ) == mst:
            for hitt in track:
                if physTools.distPtToLine(
                    physTools.pos( hitt ), ptraj_ends[ 0 ], ptraj_ends[ 1 ]
                    ) > physTools.cellWidth - 0.5:
                    break
                continue
        ...
        closest_e = physTools.distTwoLines(
            etraj_ends[ 0 ], etraj_ends[ 1 ], trk_s, trk_e
            )
        closest_p = physTools.distTwoLines(
            ptraj_ends[ 0 ], ptraj_ends[ 1 ], trk_s, trk_e
            )
    ...
    strtracklist.sort(
        key=lambda h: hit.getZPos(), reverse=True
        ) # Should be done check this
    ...
    while currentInd < len( strtracklist ):
        ...
        while tmpInd < len( strtracklist ) and not mergeFound:
            ...
            trk_e = np.array( (
                track[ -1 ].getXPos(), track[ -1 ].getYPos(),
                track[ -1 ].getZPos()
                ) )
            trk_s = np.array( (
                track[ 0 ].getXPos(), track[ 0 ].getYPos(),
                track[ 0 ].getZPos()
                ) )
